...
const SNAPSHOTS = [
  { id: "before", lap: 23, label: "Before stops" },
  { id: "mclaren", lap: 26, label: "McLaren stop cost" },
  { id: "reset", lap: 33, label: "After VER stop" },
] as const

const ORDER_LABELS = ["1st", "2nd", "3rd", "4th"]

function latestGapAtLap(
  gaps: NonNullable<QatarChartProps["race"]["chapterData"]>["gapSeries"],
  driver: DriverKey,
  lap: number
) {
  return gaps
...
export function PositionRestackChart({ race, drivers }: QatarChartProps) {
  ...
  const bumpLine = line<{ x: number; y: number }>()
    ...
    .y((point) => point.y)
    .curve(curveBumpX)
  const [hoverSnapshotId, setHoverSnapshotId] = React.useState<
    (typeof SNAPSHOTS)[number]["id"] | null
  >(null)
  const activeSnapshot =
    hoverSnapshotId === null
      ? null
      : (snapshotById.get(hoverSnapshotId) ?? null)
  const activeX =
    activeSnapshot === null ? null : (x(activeSnapshot.id) ?? null)
  const handlePointerMove = React.useCallback(
    (event: React.PointerEvent<SVGSVGElement>) => {
      ...
      const nearest = snapshots.reduce(
        (best, snapshot) => {
  ...
  return (
    <>
      ...
      >
        <title>
          Qatar road-order snapshots through the forced-stop restack
        </title>
        <PanelTitle
          x={16}
          y={30}
        ...
        {snapshots.map((snapshot) => (
          <g
            key={snapshot.id}
            transform={`translate(${x(snapshot.id) ?? 0} 0)`}
          >
        ...
        {DRIVER_ORDER.map((driver) => {
          const points = snapshots.flatMap((snapshot) => {
            ...
            return entry && typeof pointX === "number"
              ? [{ x: pointX, y: y(entry.order), entry, snapshot }]
              : []
          })
          ...
          return (
            <g key={`restack-driver-${driver}`}>
              ...
              <text
                ...
                fill={color}
              >
                {endPoint.entry.gap < 0.05
                  ? "lead"
                  : `+${endPoint.entry.gap.toFixed(1)}s`}
              </text>
            </g>
          )
        })}
        ...
        {activeSnapshot !== null && activeX !== null ? (
          <g className="chart-hover-layer">
            ...
            <g
              transform={`translate(${Math.min(540, Math.max(84, activeX + 18))} 76)`}
            >
              <rect
                width="186"
                height="102"
              ...
              {activeSnapshot.entries.map((entry, index) => (
                <g
                  key={entry.driver}
                  transform={`translate(12 ${40 + index * 14})`}
                >
                  <circle
                    r="3.5"
                    cy="-4"
