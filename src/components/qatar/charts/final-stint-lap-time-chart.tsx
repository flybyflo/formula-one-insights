...
export function FinalStintLapTimeChart({ race, drivers }: QatarChartProps) {
  const laps = (race.chapterData?.lapSeries ?? []).filter(
    (lap) => typeof lap.lapTime === "number"
  )
  const pits = race.chapterData?.pitStops ?? []
  const lapMax = qatarLapMax(race)
  const lateWindow = laps.filter(
    (lap) =>
      lap.lap >= FINAL_SPRINT_START_LAP &&
      !lap.pitOut &&
      (lap.lapTime ?? 0) < 95
  )
  const finalPaceDrivers = drivers.filter((driver) => driver.key !== "sainz")
  const lateTimeExtent = extent(lateWindow, (lap) => lap.lapTime ?? 0) as [
    number,
    number,
  ]
  ...
  const hoverSamples =
    activeHoverLap === null
      ...
      : finalPaceDrivers.flatMap((driver) => {
          const sample = lateWindow.find(
            (lap) => lap.driver === driver.key && lap.lap === activeHoverLap
          )
          return sample && typeof sample.lapTime === "number"
            ? [{ driver, lapTime: sample.lapTime }]
            : []
        })
    ...
    (event: React.PointerEvent<SVGSVGElement>) => {
      ...
      setHoverLap(
        Math.round(
          Math.min(
            lapMax,
            Math.max(FINAL_SPRINT_START_LAP, lateX.invert(viewX))
          )
        )
      )
    },
  ...
  return (
    ...
    >
      ...
      <PanelTitle
        ...
        subtitle="x = lap after Norris's final stop, y = green-flag lap time."
      />
      ...
      {finalPaceDrivers.map((driver) => {
        const series = lateWindow
          .filter(
            (lap) =>
              lap.driver === driver.key && typeof lap.lapTime === "number"
          )
          ...
          <g key={driver.key}>
            ...
            {series
              ...
              .map((sample) => (
                ...
                >
                  <title>
                    {driver.acronym} lap {sample.lap}:{" "}
                    {formatSeconds(sample.lapTime ?? 0)}s
                  </title>
                </circle>
              ))}
          </g>
      ...
      {activeHoverLap !== null && hoverX !== null ? (
        <g className="chart-hover-layer">
          ...
          <g
            transform={`translate(${Math.min(548, Math.max(94, hoverX + 14))} 88)`}
          >
            <rect
              width="178"
              height="78"
            ...
            {hoverSamples.map(({ driver, lapTime }, index) => (
              <g
                key={driver.key}
                transform={`translate(12 ${42 + index * 15})`}
              >
